...
public:
	...
	inline bool operator == (const Bstrlib::CBString& b) const { return s == b; }
	inline bool operator != (const Bstrlib::CBString& b) const { return s != b; }
	inline bool operator <  (const Bstrlib::CBString& b) const { return s <  b; }
	inline bool operator <= (const Bstrlib::CBString& b) const { return s <= b; }
	inline bool operator >  (const Bstrlib::CBString& b) const { return s >  b; }
	...
	inline operator const Bstrlib::CBString () const { return s; }

	// Accessors
	inline int length () const { return this->s.length (); }
	inline unsigned char character (int i) const { return this->s.character (i); }
...
Bstrlib::CBString v;
	...
	// CBString.
	if (t == "echo ") puts ("The echo command is about to be executed.");
