...
static int entry (void * parm, int ofs, const bstring entry) {
	struct parms * p = (struct parms *) parm;
	if (entry->slen >= p->minLen) {
		if (p->ofsfmt) printf (p->ofsfmt, ofs);
		if (p->showFilename) printf ("%s:", p->filename);
	...
	for (i=1; i < argc; i++) {
		if (argv[i][0] == '-') {
			if (argv[i][1] == 'f' || (argv[i][1] == '-' && argv[i][2] == 'p')) {
				...
				fprintf (stderr, "  -t{o,x,d}                 Print the offest of the string in base 8, 10 or 16\n");
			...
			if (NULL == infiles) infiles = bfromcstr (argv[i]);
			else {
